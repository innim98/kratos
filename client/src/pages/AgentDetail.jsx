...
export default function AgentDetail({ agentId, onGoBack }) {
  ...
  // Switching to another agent while viewing Text should drop back to Terminal
  // (the newly selected agent opens in terminal mode).
  useEffect(() => {
    setLeftTab(t => (t === 'text' ? 'terminal' : t));
    setMobileTab(t => (t === 'text' ? 'terminal' : t));
    setIdeBottomTab(t => (t === 'text' ? 'terminal' : t));
  }, [agentId]);
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 border-b border-border shrink-0">
          ...
          {agent && (
            <button
              ...
              className={cn('inline-flex items-center gap-1 text-xs', agent.status === 'working' ? 'text-emerald-500' : agent.status === 'ask' ? 'text-purple-500' : agent.status === 'idle' ? 'text-yellow-500' : agent.status === 'online' ? 'text-emerald-500' : 'text-muted-foreground')}
            >
              <span className={cn('h-1.5 w-1.5 rounded-full', agent.status === 'working' ? 'bg-emerald-500 animate-pulse' : agent.status === 'ask' ? 'bg-purple-500 animate-pulse' : agent.status === 'idle' ? 'bg-yellow-500' : agent.status === 'online' ? 'bg-emerald-500' : 'bg-muted-foreground/50')} />
              {agent.status}
            </button>
          )}
    ...
    <div className="flex items-center justify-between px-2 py-1.5 border-b border-border shrink-0">
      <div className="flex items-center gap-2">
        ...
        {agent && !editing && (
          <button
            onClick={() => setStatusOpen(true)}
            className={cn('inline-flex items-center gap-1 text-xs cursor-pointer hover:underline', agent.status === 'working' ? 'text-emerald-500' : agent.status === 'ask' ? 'text-purple-500' : agent.status === 'idle' ? 'text-yellow-500' : agent.status === 'online' ? 'text-emerald-500' : 'text-muted-foreground')}
          ...
          <div ref={ideRightRef} className="flex-1 flex flex-col min-w-0 min-h-0">
            ...
            <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
              <div className="flex items-center gap-0.5 px-1.5 py-1 border-b border-border bg-card/50 shrink-0">
                ...
                <UploadForAgent agentId={agentId} onSendToInput={(path) => termRef.current?.insertToInput(path)} />
              </div>
  ...
  const uploadActions = <UploadForAgent agentId={agentId} onSendToInput={(path) => termRef.current?.insertToInput(path)} />;
